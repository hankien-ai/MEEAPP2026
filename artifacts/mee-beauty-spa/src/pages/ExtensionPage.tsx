...
const ExtensionPage: React.FC<ExtensionPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#F8FAFC] p-4 pb-12 sm:max-w-2xl lg:max-w-4xl">
        ...
        <div className="mb-5 bg-white/80 border border-slate-200/60 p-3.5 rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] backdrop-blur-md">
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {quickModules.map((mod) => (
              ...
              >
                <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-50 rounded-2xl flex items-center justify-center border border-blue-100/80 text-blue-600 group-hover:bg-blue-100 transition-all shadow-sm group-hover:shadow-md">
      ...
      {/* Chế độ sửa Quick */}
      {isEditQuick && (
        <div className="mb-4 bg-blue-50/80 border border-blue-100 rounded-2xl p-3 flex items-center justify-between text-xs text-blue-900">
          <span className="font-medium">Chạm ứng dụng bên dưới để thêm/bớt Quick</span>
          <span className="font-bold bg-blue-200/50 text-blue-700 px-2 py-0.5 rounded-full text-[10px]">
              ...
              >
                <div
                  className={`relative w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center transition-all ${
                    isQuick && !isEditQuick
                      ? 'bg-blue-50 text-blue-600 border border-blue-200 shadow-sm group-hover:bg-blue-100'
                ...
                >
                  ...
                  {mod.adminOnly && !isEditQuick && (
                    <span className="absolute -top-1 -right-1 bg-blue-500 text-white text-[8px] font-bold px-1 rounded-full border border-white">
                      ADM
                    </span>
                  )}
                  {isEditQuick && (
                    <div
                      className={`absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow-sm transition-all ${
                        isQuick ? 'bg-blue-600 text-white scale-105' : 'bg-slate-200 text-slate-500'
                      }`}
                    >
