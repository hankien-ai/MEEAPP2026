...
export const DashboardPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "PAID":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 shrink-0">
            Đã thanh toán
          </span>
        );
      case "PARTIALLY_PAID":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-amber-100 text-amber-800 border border-amber-200 shrink-0">
            Thanh toán 1 phần
          </span>
        );
      case "DRAFT":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200 shrink-0">
            Đơn nháp
          </span>
        );
      case "VOID":
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-rose-100 text-rose-800 border border-rose-200 shrink-0">
            Đã hủy
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-bold bg-slate-100 text-slate-600 shrink-0">
            {status || "Khác"}
          </span>
        );
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-3 sm:p-6 font-sans text-slate-800 space-y-4 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-teal-600 text-white flex items-center justify-center font-black text-lg sm:text-xl shadow-md shadow-teal-600/20 shrink-0">
            M
          </div>
          <div className="min-w-0">
            <h1 className="font-extrabold text-slate-900 text-base sm:text-xl tracking-tight truncate">
              MEE BEAUTY SPA
            </h1>
            <p className="text-[11px] sm:text-xs text-slate-500 truncate">
              Báo cáo doanh thu & hoạt động hôm nay
            </p>
          </div>
        </div>

        <div className="flex items-center justify-between sm:justify-end gap-2 border-t sm:border-t-0 pt-2.5 sm:pt-0 border-slate-100">
          <span className="text-[10px] sm:text-[11px] font-semibold px-2.5 py-1 bg-teal-50 text-teal-700 rounded-lg border border-teal-100 shrink-0">
      ...
      {/* ERROR BANNER */}
      {errorMsg && (
        <div className="bg-rose-50 border border-rose-200 text-rose-800 p-3.5 rounded-2xl text-xs font-semibold flex items-center justify-between gap-3">
      ...
      {!loading && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2.5 sm:gap-4">
            {/* 1. DOANH THU HÔM NAY */}
            <div className="col-span-2 sm:col-span-2 lg:col-span-1 xl:col-span-2 bg-gradient-to-br from-teal-600 to-emerald-700 text-white p-4 rounded-2xl shadow-md shadow-teal-600/10 flex flex-col justify-between space-y-2">
              <div className="flex justify-between items-center opacity-90">
                <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                  Doanh Thu Hôm Nay
                </span>
                <span className="p-1 bg-white/20 rounded-lg text-xs">💵</span>
              </div>
              <div>
            ...
            {/* 2. ĐƠN HÀNG */}
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-2">
              <div className="flex justify-between items-center text-slate-500">
                <span className="text-[10px] font-bold uppercase tracking-wider">
                  Đơn Hàng
                </span>
                <span className="p-1 bg-slate-100 rounded-lg text-xs">🛍️</span>
              </div>
              <div>
            ...
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-2">
              <div className="flex justify-between items-center text-slate-500">
                ...
                <span className="p-1 bg-blue-50 text-blue-600 rounded-lg text-xs">
                  ✨
                </span>
              </div>
              <div>
                <div className="text-base sm:text-xl font-extrabold text-blue-700">
                  {stats.servicesSold}{" "}
                  <span className="text-[10px] sm:text-xs font-normal text-slate-500">
                    lượt
                  </span>
            ...
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-2">
              <div className="flex justify-between items-center text-slate-500">
                ...
                <span className="p-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs">
                  📦
                </span>
              </div>
              <div>
                <div className="text-base sm:text-xl font-extrabold text-emerald-700">
                  {stats.productsSold}{" "}
                  <span className="text-[10px] sm:text-xs font-normal text-slate-500">
                    món
                  </span>
            ...
            {/* 6. GÓI LIỆU TRÌNH */}
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between space-y-2 col-span-2 sm:col-span-1">
              <div className="flex justify-between items-center text-slate-500">
                <span className="text-[10px] font-bold uppercase tracking-wider">
                  Gói Liệu Trình
                </span>
                <span className="p-1 bg-purple-50 text-purple-600 rounded-lg text-xs">
                  🎁
                </span>
              </div>
              <div>
                <div className="text-base sm:text-xl font-extrabold text-purple-700">
                  {stats.packagesSold}{" "}
                  <span className="text-[10px] sm:text-xs font-normal text-slate-500">
                    gói
                  </span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
            ...
            {recentInvoices.length === 0 ? (
              <div className="p-8 sm:p-12 text-center text-slate-400 space-y-1">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[550px]">
                  <thead>
                    <tr className="bg-slate-50/80 border-b border-slate-100 text-[10px] sm:text-[11px] font-bold uppercase text-slate-500 tracking-wider">
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                    {recentInvoices.map((inv) => (
                      ...
                      >
                        <td className="py-2.5 px-3 sm:px-4 font-mono font-bold text-slate-800">
                          #{inv.id ? inv.id.slice(0, 8).toUpperCase() : "---"}
                        </td>
                        <td className="py-2.5 px-3 sm:px-4 text-slate-500 text-[11px]">
                        ...
                        <td className="py-2.5 px-3 sm:px-4 text-slate-600 text-[11px]">
